...
{
	const Vector3 eyePos(84966.2031f, 103187.719f, 12428.7969f);
	const Vector3 lookAt(84877.8594f, 0.000000000f, 38398.2070f);
...
void cMapView::OnPreRender(const float deltaSeconds)
{
	...
	if (m_renderTarget.Begin(renderer))
	{
		...
		m_quadTree.Render(renderer, frustum, 5, 15, ray1, ray2);
	}
...
void cMapView::OnRender(const float deltaSeconds) 
{
	...
	if (ImGui::Begin("Information", &isOpen, ImVec2(500.f, 120.f), windowAlpha, flags))
	{
		...
		//ImGui::SameLine();
		//ImGui::Checkbox("Texture", &m_quadTree.m_isShowTexture);
		//ImGui::SameLine();
		ImGui::Checkbox("Quadtree", &m_quadTree.m_isShowQuadTree);
		//ImGui::Checkbox("Light", &m_quadTree.m_isLight);
		ImGui::SameLine();
